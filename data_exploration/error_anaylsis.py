...
custom_palette = ['#FFDAB9', '#9CAF88', '#e7d1ff', '#93ccea']
...
pdir = "0_baseline"
#%%
def boxplot(df, column, by, title, y_label:str="sequence length", x_label:str="scores", fname="boxplot.png"):
    ...
    ax.yaxis.grid(True, linestyle='dotted', linewidth=1.1)
    # Set titles and labels
    # plt.title(f'{title} - Boxplot of values by {by}', fontsize=16)
    plt.xlabel(f'Group: {x_label}', fontsize=14)
    plt.ylabel(y_label, fontsize=14)
    
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    import os
    os.makedirs(os.path.join(f"./{pdir}", "plots", "boxplots"), exist_ok=True)
    plt.savefig(os.path.join(f"./{pdir}", "plots", "boxplots", fname))
    plt.show()


#%%
def get_dataset(lang:str="en"):
    path = f"{f'{lang}_' if lang == 'en' else ''}{pdir}\outputStandardFileBaseline_{lang}.json"
...
#%%
boxplot(q_all, by='pred_length', column='score_quantile', y_label="sequence length: predicted response", title="lang: all", fname="pred_len_all.png")
boxplot(q_all, by='target_length', column='score_quantile', y_label="sequence length: target response",title="lang: all", fname="target_len_all.png")
boxplot(q_all, by='length_diff', column='score_quantile', y_label="sequence abs length diff: target vs. predicted response",title="lang: all", fname="diff_all.png")
...
#%%
boxplot(q_vi, by='pred_length', column='score_quantile', y_label="sequence length: predicted response", title="lang: vi",  fname="pred_len_vi.png")
boxplot(q_vi, by='target_length', column='score_quantile', y_label="sequence length: target response",title="lang: vi", fname="target_len_vi.png")
boxplot(q_vi, by='length_diff', column='score_quantile', y_label="sequence abs length diff: target vs. predicted response", title="lang: vi", fname="diff_vi.png")

#%%
boxplot(q_fr, by='pred_length', column='score_quantile', y_label="sequence length: predicted response",  title="lang: fr", fname="pred_len_fr.png")
boxplot(q_fr, by='target_length', column='score_quantile', y_label="sequence length: target response",title="lang: fr",  fname="target_len_fr.png")
boxplot(q_fr, by='length_diff', column='score_quantile', y_label="sequence abs length diff: target vs. predicted response",title="lang: fr", fname="diff_fr.png")
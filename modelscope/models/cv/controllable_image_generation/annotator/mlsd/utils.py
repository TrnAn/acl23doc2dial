...
def deccode_output_score_and_ptss(tpMap, topk_n=200, ksize=5):
    ...
    hmax = F.max_pool2d(
        heat, (ksize, ksize), stride=1, padding=(ksize - 1) // 2)
...
def pred_squares(image, model, input_shape=[512, 512], params=params_glob):
    '''
    shape = [height, width]
    '''
    h, w, _ = image.shape
    original_shape = [h, w]
    temp_img01 = cv2.resize(
        image, (input_shape[0], input_shape[1]), interpolation=cv2.INTER_AREA)
    resized_image = np.concatenate(
        [temp_img01, np.ones([input_shape[0], input_shape[1], 1])], axis=-1)
    resized_image = resized_image.transpose((2, 0, 1))
    batch_image = np.expand_dims(resized_image, axis=0).astype('float32')
    batch_image = (batch_image / 127.5) - 1.0
    ...
    dist_inter_to_segment1 = np.sort(
        np.concatenate(
            [dist_inter_to_segment1_start, dist_inter_to_segment1_end],
            axis=-1),
        axis=-1)  # [n_batch, n_batch, 2]
    dist_inter_to_segment2 = np.sort(
        np.concatenate(
            [dist_inter_to_segment2_start, dist_inter_to_segment2_end],
            axis=-1),
        axis=-1)  # [n_batch, n_batch, 2]
    ...
    # sort deg ascending
    deg_sort = np.sort(
        np.concatenate([deg1_map[:, :, None], deg2_map[:, :, None]], axis=-1),
        axis=-1)
    ...
    for i in range(inter_pts.shape[0]):
        for j in range(i + 1, inter_pts.shape[1]):
            ...
            temp01 = dist_inter_to_segment1[i, j, 1] >= dist_segments[i]
            temp02 = dist_inter_to_segment1[
                i, j, 0] <= dist_segments[i] * outside_ratio
            temp03 = dist_inter_to_segment1[i, j, 1] <= dist_segments[i]
            temp04 = dist_inter_to_segment1[
                i, j, 0] <= dist_segments[i] * inside_ratio
            temp_cond01 = ((temp01 and temp02) or (temp03 and temp04))
            temp05 = dist_inter_to_segment2[i, j, 1] >= dist_segments[j]
            temp06 = dist_inter_to_segment2[
                i, j, 0] <= dist_segments[j] * outside_ratio
            temp07 = dist_inter_to_segment2[i, j, 1] <= dist_segments[j]
            temp08 = dist_inter_to_segment2[
                i, j, 0] <= dist_segments[j] * inside_ratio
            temp_cond02 = ((temp05 and temp06) or (temp07 and temp08))
            check_distance = temp_cond01 and temp_cond02
                            ...
                            if connect23:
                                for corner3_line in corner3[2:]:
                                    if corner3_line in corner0[2:]:
                                        ...
                                        segments_list:
                                            order: 0 > 1 > 2 > 3
                                            | line_idx0_i, line_idx0_j, line_idx1_i, line_idx1_j,
                                            line_idx2_i, line_idx2_j, line_idx3_i, line_idx3_j |
                                            | line_idx0_i, line_idx0_j, line_idx1_i, line_idx1_j,
                                            line_idx2_i, line_idx2_j, line_idx3_i, line_idx3_j |
                                            ...
                                        '''
    ...
    try:
        ...
        correction = area_x[:, -1] * area_y[:, 0] - area_y[:, -1] * area_x[:,
                                                                           0]
        area_scores = np.sum(
            area_x[:, :-1] * area_y[:, 1:], axis=-1) - np.sum(
        ...
        temp_value001 = params['w_overlap'] * overlap_scores
        temp_value002 = params['w_degree'] * degree_scores
        temp_value003 = params['w_area'] * area_scores
        temp_value004 = params['w_center'] * center_scores
        temp_value005 = params['w_length'] * length_scores
        score_array = temp_value001 + temp_value002 + temp_value003 - temp_value004 + temp_value005

            ...
            if (nA != 0 and nB != 0):
                ...
                for i in range(nA):
                    for j in range(nB):
                        ...
                        vec_x = np.array([
                            score_mid[int(round(startend[Ivalue2][1])),
                                      int(round(startend[Ivalue2][0])), 0]
                            for Ivalue2 in range(len(startend))
                        ])
                        vec_y = np.array([
                            score_mid[int(round(startend[Ivalue1][1])),
                                      int(round(startend[Ivalue1][0])), 1]
                            for Ivalue1 in range(len(startend))
                        ])
            ...
            if k not in special_k:
                ...
                for i in range(len(connection_all[k])):  # = 1:size(temp,1)
                    ...
                    if found == 1:
                        ...
                        j1, j2 = subset_idx
                        temp_value01 = (subset[j1] >= 0).astype(int)
                        temp_value02 = (subset[j2] >= 0).astype(int)
                        membership = (temp_value01 + temp_value02)[:-2]

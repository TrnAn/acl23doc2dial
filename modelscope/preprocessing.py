...
def save_to_json(df:pd.DataFrame, export_cols:list, fname:str="dev.json", dir:str="", force_ascii:bool=True):
    dir = os.path.join(dir, fname)

    logger.info(f"save test set {fname}...")
    df[export_cols].to_json(fname, orient="records", lines=True, force_ascii=force_ascii)
    logger.info("DONE...")
...
def save_as_formatted_dev(fname:str, save_dir:str="."):
    with open(fname) as f_in:
        with open(f'{save_dir}/{f_name}_input.jsonl', 'w') as f_out:
